...
TesseractAPIPtr initGlobalAPI() {
  if(_api->Init(NULL, "rus+eng", tesseract::OEM_LSTM_ONLY) != 0) { CV_WARNING("Failed to init a TesseractOCR API instance."); }
...
cv::String extractTextFromImage(const cv::Mat& mat, TesseractAPIPtr api) {
  ...
  cv::resize(img, img, cv::Size(internal::constant::SZ_imageWidth, internal::constant::SZ_imageHeight));
  ...
  std::sort(conts.begin(), conts.end(), [](const std::vector<cv::Point>& lhs, const std::vector<cv::Point>& rhs) {
  ...
  for(auto start = conts.begin(); start != conts.end();) {
    ...
    if(r.width > img.size().width * internal::constant::LIMIT_imageMaxWidthCoef ||
       ...
       (!isInternal && r.width < internal::constant::LIMIT_imageMinWidthCoef * img.size().width) ||
       (!isInternal && r.height < internal::constant::LIMIT_imageMinHeightCoef * img.size().height) ||
       (isInternal && r.width < internal::constant::LIMIT_imageMinWidthInternalCoef * img.size().width) ||
       (isInternal && r.height < internal::constant::LIMIT_imageMinHeightInternalCoef * img.size().height) || r.width / 2 > r.height || //
       r.x == 0 || r.y == 0) {
      conts.erase(start);
      continue;
    }

    cv::fillPoly(temp, std::vector<std::vector<cv::Point>>{*start}, cv::Scalar(255, 255, 255));
    ++start;
  }
  ...
  for(auto con : conts) {
    ...
    cv::Mat contImg =
        temp(cv::Rect(r.x >= 5 ? r.x - 5 : r.x, r.y >= 5 ? r.y - 5 : r.y, r.width + (r.x >= 5 ? 10 : 0), r.height + (r.y >= 5 ? 10 : 0)));
    ...
    if(index == 0 || index == 4 || index == 5) {
      api->SetVariable("tessedit_char_whitelist", internal::constant::WL_characters.data());
    } else {
    ...
    cv::String res =
        internal::searchTextOnDifferentSizes(api, contImg.clone(), contImg.size().width, contImg.size().height, internal::DF_Height);
    if(res.empty()) {
      res = internal::searchTextOnDifferentSizes(api, contImg.clone(), contImg.size().width, contImg.size().height, internal::DF_Width);
    }

    if(res.empty()) { res = internal::searchTextOnBluredImage(api, contImg.clone()); }
...
cv::String searchTextOnBluredImage(std::shared_ptr<tesseract::TessBaseAPI> tessApi, cv::Mat mat, int startLevel) {
  cv::String result;
  do {
    if(startLevel > constant::BLUR_MaxLevel) { break; }

    while((startLevel * startLevel) % 2 == 0) { ++startLevel; }
...
cv::String searchTextOnDifferentSizes(std::shared_ptr<tesseract::TessBaseAPI> tessApi,
  ...
  cv::String result;
  do {
    if(((side == DF_Width || side == DF_WidthHeight) && width > constant::DF_imageMaxSideWidth) ||
    ...
    if(side == DF_Width || side == DF_WidthHeight) { width += upCoefVal; }
    if(side == DF_Height || side == DF_WidthHeight) { height += upCoefVal; }
...
bool isInternalContour(const std::vector<std::vector<cv::Point>>& contours, const std::vector<cv::Point>& contour) {
  ...
  std::remove_reference_t<decltype(contours)>::const_iterator found =
      std::find_if(contours.cbegin(), contours.cend(), [bContour](const std::vector<cv::Point>& po) {
...
cv::String textFromImage(std::shared_ptr<tesseract::TessBaseAPI> tessApi, cv::Mat mat) {
  ...
  if(textLen != 0 && text[0] != ' ') { result = text::utf82Str(text, textLen); }
  delete[] text;
